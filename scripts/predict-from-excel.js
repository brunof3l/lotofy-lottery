...
function readExcelResults(filePath) {
  ...
  // Detect header row: the first row containing 'concurso' or 'data' (pt-BR)
  ...
  for (let r = headerRowIndex + 1; r < rows.length; r++) {
    const row = rows[r] || [];
    const rawContest = row[contestIdx];
    ...
    // Extract all numbers 1..25 from row
    const nums = [];
    const seen = new Set();
...
function computeFrequency(results) {
  ...
  return freq.slice(1); // index 0 unused
}
...
function generatePredictions(results) {
  ...
  // Statistical: top 15 by frequency
  const topAll = freq
    .map((f, i) => ({ n: i + 1, f }))
  ...
  // Balanced: 3 numbers from each quintile range
  const balanced = [];
  ...
  // Hot: emphasize recently frequent numbers (same as top for simplicity)
  const hot = topAll;

  // Random: 15 unique random numbers
  const random = [];
  const rused = new Set();
...
function main() {
  try {
    ...
    if (results.length === 0) {
      console.log(JSON.stringify({ error: 'Nenhum resultado válido encontrado no Excel.' }, null, 2));
      return;
    }
